...
art="/home/trycatch/.config/ario/covers/$(ls ~/.config/ario/covers | "\
...
(echo "^fg($highlight)Music"; echo "       "; \
 echo "^ca(1,/home/trycatch/.xmonad/scripts/dzen_lyrics.sh) " \
      "Track:  ^fg($highlight)$track^ca()"; \
 echo "^ca(1,/home/trycatch/.xmonad/scripts/dzen_artistinfo.sh)^fg() "\
      "  Artist: ^fg($highlight)$artist^ca()"; \
 echo "^ca(1,/home/trycatch/.xmonad/scripts/dzen_albuminfo.sh)^fg()  "
 "Album:  ^fg($highlight)$album^ca()"; \
 echo "^ca(1,/home/trycatch/.xmonad/scripts/dzen_lyrics.sh)  "\
      "Year:   ^fg($highlight)$date^ca()"; 
 echo "  $percbar"; \
 echo "      ^ca(1, ncmpcpp prev)  "\
      "^fg($white0)^i(/home/trycatch/.xmonad/dzen2/prev.xbm) "\
      "^ca()  ^ca(1, ncmpcpp pause) ^i(/home/trycatch/.xmonad/dzen2/pause.xbm) "\
      "^ca()  ^ca(1, ncmpcpp play) ^i(/home/trycatch/.xmonad/dzen2/play.xbm) "\
      "^ca()   ^ca(1, ncmpcpp next) ^i(/home/trycatch/.xmonad/dzen2/next.xbm) ^ca()"; \
 echo " "; sleep 15) | \
    dzen2 \
        -fg $foreground \
        -bg $background \
